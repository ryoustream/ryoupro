...
import com.rynime.nvplayer.ui.navigation.AppNavigation
import com.rynime.nvplayer.ui.theme.NosvedPlayerTheme
...
class MainActivity : ComponentActivity() {
    ...
    private val pipReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent.action) {
                "com.rynime.nvplayer.PIP_PREV" -> playerViewModel.playPrevious()
                "com.rynime.nvplayer.PIP_PLAY_PAUSE" -> {
                    playerViewModel.togglePlayback()
                    updatePipActions()
                }
                "com.rynime.nvplayer.PIP_NEXT" -> playerViewModel.playNext()
            }
        }
    }
    ...
    private fun buildPipActionsList(): List<android.app.RemoteAction> {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // 1. Prev Action
            val prevIntent = Intent("com.rynime.nvplayer.PIP_PREV")
            ...
            val playPauseIntent = Intent("com.rynime.nvplayer.PIP_PLAY_PAUSE")
            ...
            // 3. Next Action
            val nextIntent = Intent("com.rynime.nvplayer.PIP_NEXT")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                com.rynime.nvplayer.player.ytdlp.YtdlpManager.copyAssets(applicationContext)
        ...
        val filter = IntentFilter().apply {
            addAction("com.rynime.nvplayer.PIP_PREV")
            addAction("com.rynime.nvplayer.PIP_PLAY_PAUSE")
            addAction("com.rynime.nvplayer.PIP_NEXT")
        }
    ...
    private fun logExternalVideoHistory(uri: Uri) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val metadataResult = com.rynime.nvplayer.data.mediainfo.MediaInfoOps.extractBasicMetadata(
                    this@MainActivity,
                    uri,
                    fileName
                )
                ...
                val video = com.rynime.nvplayer.domain.model.Video(
                    uri = uri.toString(),
                    title = fileName,
                    duration = duration,
                    folderName = "",
